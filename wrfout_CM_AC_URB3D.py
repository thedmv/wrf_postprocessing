...
ncfobjs = [nc.netcdf_file(ncf) for ncf in ncflist]
ac = np.array(np.squeeze([fobj.variables['CM_AC_URB3D'].data for fobj in ncfobjs]))
ac_daytotal = np.sum(ac, axis = 0)
var = ac_daytotal # VARIABLE
...
figname = './wrfvar_CM_AC_URB3D' + "__" + datetitle
plt.title('Daily Total CM_AC_URB3D (W-h/m2) ' + datetitle)
